...
function PaymentContent() {
    ...
    useEffect(() => {
        if (!participantId) {
            // Automatic redirect currently disabled to prevent flicker on load if param is missing for a split second 
            // or we can allow manual entry if user has an ID.
        }
    }, [participantId]);
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            const res = await fetch('/api/payment/order', {
                ...
                body: JSON.stringify({ participantId, amount: 400 }), // ₹400
            });
    ...
    if (!participantId) {
        return (
            <div className="text-center text-luxury-silver">
                ...
                <Button onClick={() => router.push('/register')} variant="outline" className="mt-4">Go to Registration</Button>
            </div>
        );
    }
    ...
    return (
        <div className="w-full max-w-md bg-white/5 border border-white/10 p-8 rounded-2xl text-center shadow-2xl">
            <h1 className="text-2xl font-display text-luxury-gold mb-6">Complete Registration</h1>

            <div className="bg-luxury-black p-6 rounded-xl border border-white/5 mb-8 text-left">
                <div className="flex justify-between items-end mb-2">
                    <p className="text-luxury-silver text-sm uppercase tracking-wider">Registration Fee</p>
                    <p className="text-3xl text-white font-bold font-display">₹400.00</p>
                </div>
                <div className="h-[1px] w-full bg-white/10 my-4" />
                <p className="text-xs text-luxury-silver/60">Includes full event access, welcome kit, and gala dinner.</p>
            </div>
            ...
            <p className="text-[10px] text-luxury-silver/40 uppercase tracking-widest">Secured by Razorpay</p>
        </div>
    );
}
